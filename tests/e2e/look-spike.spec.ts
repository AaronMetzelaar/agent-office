...
import { checkDesign, type Design } from '../../src/shared/looks'
...
const shots = join(spike, 'shots', model)
const accents = [0x0891b2, 0xd97706, 0x7c3aed, 0x059669, 0xdc2626, 0x2563eb]

test.skip(process.env.AGENT_OFFICE_LOOK_SHOTS !== '1' || !existsSync(join(spike, model)), 'Set AGENT_OFFICE_LOOK_SHOTS=1 after running tests/main/look-spike.test.ts')
...
test(`draws the ${model} looks from the generation spike`, async () => {
  ...
  const looks = readdirSync(join(spike, model)).map((file) => {
    const attempts = JSON.parse(readFileSync(join(spike, model, file), 'utf8')).attempts as { design?: Design }[]
    const design = [...attempts].reverse().find((a) => a.design && !checkDesign(a.design).length)?.design ?? attempts.find((a) => a.design)?.design
    return { repo: file.replace(/\.json$/, ''), design }
  }).filter((look): look is { repo: string; design: Design } => !!look.design)
  ...
  db.saveSetting('rooms', looks.map((look, i): RoomDef => ({ id: `r-look${i}`, name: look.repo, subtitle: `~/code/${look.repo}`, accent: accents[i % accents.length]!, look: 'plain', root: repos[i]!, createdAt: Date.now() - (looks.length - i) * 1000, design: look.design })))
  db.close()
  ...
  for (const look of looks) await expect(page.locator('.sign', { hasText: look.repo })).toBeVisible()
  mkdirSync(shots, { recursive: true })
  await page.waitForTimeout(3000)
  await page.screenshot({ path: join(shots, 'floor.png') })
  for (const look of looks) {
    await page.locator('.sign', { hasText: look.repo }).click()
