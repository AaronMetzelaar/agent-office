...
test('the terminal button keeps a shell in the chat folder open, even after leaving the chat, until it is hidden', async () => {
  const terminal = page.getByRole('region', { name: 'Terminal' })
  const toggle = drawer().getByRole('button', { name: 'Terminal', exact: true })
  await toggle.click()
  await expect(toggle).toHaveAttribute('aria-pressed', 'true')
  await expect(terminal).toContainText(folder.split('/').pop()!)
  await terminal.locator('.xterm').click()
  await page.keyboard.type('echo "shell in $(basename $PWD)"\n')
  await expect(terminal.locator('.xterm-rows')).toContainText(`shell in ${folder.split('/').pop()}`, { timeout: 10_000 })
  await drawer().getByRole('button', { name: 'Back to inbox' }).click()
  await expect(terminal).toHaveCount(0)
  await drawer().locator('.standby summary', { hasText: 'Standby' }).click({ position: { x: 12, y: 12 } })
  await drawer().locator('.standby .brow', { hasText: 'Tidy the bid flow' }).click()
