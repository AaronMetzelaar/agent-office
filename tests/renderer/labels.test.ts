...
function overview(samples: Sample[], zoomed: boolean) {
  ...
  const desks = assignDesks(new Map(), samples, (id) => capacityOf(id, demand[id] ?? 0))
  ...
  const signs = deptIds
    ...
    .map((id) => {
      const [x, y] = screen(dept[id].sign[0] + floor.zones[id].ox, 0.62, dept[id].sign[1])
      return [x, y - 40, x + 30 + dept[id].name.length * 6.5, y] as [number, number, number, number]
    })
  const [px, py] = screen(parkedZone.sign[0], 0.62, parkedZone.sign[1])
  signs.push([px, py - 40, px + 80, py])
  ...
  for (const s of samples) {
    ...
    const [bx, bz] = floor.zones[s.dept].slots[desks.get(s.id)!]!
