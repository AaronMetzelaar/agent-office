...
export const pictures = ['shirt', 'pitch', 'dashboard', 'chart', 'code', 'app'] as const
...
export const limits = { props: 14, perTier: [0, 5, 8, 12], parts: 24, allParts: 160, height: 2.8, span: 2.6, labels: 6, label: 18, backDepth: 1.45, sideWidth: 1.5, stretch: 4, stage: { width: 2.1, depth: 1.8 } }
...
function checkPart(where: string, part: Part): string[] {
  ...
  for (const [color, large] of [[part.color, big], [part.textColor, false], [part.accent, false]] as const) {
  ...
  if (part.picture !== undefined && (part.shape !== 'panel' || !pictures.includes(part.picture))) out.push(`${where} has a picture, but only panels show one of ${pictures.join(', ')}`)
...
export function arrange(design: Design, tier: 1 | 2 | 3): Prop[] {
  const { w, d } = roomSize(tier), g = deskGrid, gap = 0.2, edge = 0.2, stageX = g.left + g.cw
  const backEnd = tier >= 2 ? w - g.side - g.right - 0.05 : w - edge
  ...
  const fit = (prop: Prop, depth: number, width: number, up = 1) => {
    const [x0, z0, x1, z1] = footprint(prop), top = Math.max(...prop.parts.map((part) => part.at[1] + part.size[1] / 2))
    const k = Math.min(Math.max(1, up / (x1 - x0)), 1.8, limits.height / top, depth / (z1 - z0), width / (x1 - x0), limits.span / (x1 - x0))
  ...
  const back = here.filter((p) => p.zone === 'back')
  const star = back.find((p) => p.signature)
  let segments: [number, number][] = [[edge, backEnd]]
  if (star) {
    const f = fit(star, limits.stage.depth, limits.stage.width, limits.stage.width)
    out.push({ ...f.prop, x: stageX - f.x0 - f.fw / 2, z: 0.1 - f.z0 })
    segments = [[edge, stageX - f.fw / 2 - gap], [stageX + f.fw / 2 + gap, backEnd]]
  }
  for (const lane of ['floor', 'wall'] as const) {
    const rows = segments.map(() => [] as ReturnType<typeof fit>[]), used = segments.map(() => 0)
    for (const prop of back.filter((p) => p !== star && (lane === 'wall') === hanging(p))) {
      const free = segments.map(([a, b], j) => b - a - used[j]! - (rows[j]!.length ? gap : 0))
      const k = free.indexOf(Math.max(...free))
      if (free[k]! < 0.5) continue
      const f = fit(prop, limits.backDepth - 0.15, free[k]!)
      rows[k]!.push(f)
      used[k]! += f.fw + (rows[k]!.length > 1 ? gap : 0)
    }
    rows.forEach((row, j) => {
      const [a, b] = segments[j]!, spacing = (b - a - row.reduce((n, f) => n + f.fw, 0)) / (row.length + 1)
      let at = a + spacing
      for (const f of row) {
...
export function tidy(design: Design): Design {
  ...
  const part = (source: Part): Part => {
    ...
    if (next.accent !== undefined && !hex.test(next.accent)) delete next.accent
    if (next.picture !== undefined && (next.shape !== 'panel' || !pictures.includes(next.picture))) delete next.picture
  ...
  for (const tier of tiers) {
    ...
    for (const w of [narrow, narrow + limits.stretch]) {
      ...
      here.forEach((prop) => {
        ...
        if (prop.zone === 'back' && box[3] > (prop.signature ? limits.stage.depth + 0.15 : limits.backDepth)) out.push(`${where} reaches ${box[3].toFixed(2)} m from the back wall; the back zone ends at ${limits.backDepth} m`)
        ...
        properties: {
          ...
          parts: {
            ...
            items: {
              ...
              properties: { shape: { enum: [...shapes] }, size: vec, at: vec, rot: vec, color: { type: 'string' }, finish: { enum: [...finishes] }, taper: { type: 'number' }, text: { type: 'string' }, textColor: { type: 'string' }, picture: { enum: [...pictures] }, accent: { type: 'string' } },
            },
          },
        },
      },
