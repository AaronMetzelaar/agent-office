...
export async function submit(api: SendApi, chatId: string, text: string, waiting?: Pick<PendingRequestView, 'id'>): Promise<SendOutcome> {
  if (waiting) {
  ...
  const refused = await api.sendMessage(chatId, text)
