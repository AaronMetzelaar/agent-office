...
import { deptOf, depts, door, gymCooler, kindOf, layoutFloor, loungeSeat, minWidth, queueSpots, shellOf, ZF, type Bounds, type Box, type DeptDef, type DeptId, type Floor, type YardSide } from './layout'
...
export function createWorld({ scene, renderer, camera, labelsEl, region, ui, reduce, onNewDesk, onAction, removable = () => false }: WorldOptions) {
  ...
  function renderSigns() {
    ...
    for (const [id, sign] of signs) {
      ...
      } else renderSign(sign, names.get(id) ?? deptOf(id).name, countsFor(act.filter((l) => l.facts.dept === id && l.spot !== 'lounge').map((l) => l.facts)), 'No agents', on, dim)
