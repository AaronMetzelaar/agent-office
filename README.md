...
Code blocks in a reply are syntax highlighted (highlight.js, with the common languages registered; a block without a language is detected when it's short enough) and end in a bar with the language and a Copy button. Shell blocks (`sh`, `bash`, `zsh`, `console` or no language) also have Run, which types the command into a login shell in the chat's folder, shown in a terminal next to the chat. The terminal button in the chat header, or ⌃`, opens that shell without running anything. Once shown, the terminal stays open for whichever chat you open, across restarts, until you hide it; the office view shrinks to make room. Its header resumes the session in the Terminal app. Each chat keeps one shell in the agent host, so hiding the terminal leaves it running and the next Run reuses it. Close ends it. Done removes the chat's worktree; one with uncommitted changes asks first.
