...
With a Jev key saved in Accounts (a TypeSafe API key), Jev picks the room for every agent you start, based on its folder, its prompt and any Linear ticket. It chooses Marketplace, Admin, Mobile, Backend / infra, a room Claude made earlier, or a new room. For a new room, Haiku names it and writes one line on what belongs there. The agent starts in its folder's section and walks over once the room exists. A room made for a repository no section covers also takes that repository's outside chats and file activity. There are six spare rooms. Once all six are taken, Side projects replaces the new room option. When Jev is under 50% confident, fails or takes longer than 3 seconds, the agent goes to the desk you clicked or its folder's section. Research accounts go through Jev too and sit in the room it picks, with an account badge. The Research gym only takes them when Jev has no answer. Reviews skip Jev. File activity can still move the agent afterwards. Made rooms are kept in the `rooms` setting in `office.db`.
