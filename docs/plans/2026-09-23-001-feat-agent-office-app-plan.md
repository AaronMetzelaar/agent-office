...
- **Packing, the Lounge and the playground** (revised 2026-09-24, second pass):
  ...
  - The Lounge is its own scene: a rug, a coffee corner anchored to its right edge and one armchair per chat (third pass below), baked in one group and rebuilt when the count changes. Seats fill column by column from the front, so a growing Lounge never moves a seated agent.
...
- **Stable floor, reserved desks and Done** (revised 2026-09-24, third pass):
  ...
  - A chat holds a desk while it's at it, and keeps it while it rests in the Lounge or at the gym cooler. It loses it when it parks, changes department or disappears. A chat first seen resting (a relaunch, or a visitor showing up) gets a desk only when it was active in the last day. A section's size is fixed until its set of holders changes; it's then holders + 1, or more if a seated agent sits at a higher slot. Away holders with too-high slots are renumbered, which nobody sees, and seated agents never are.
  - Every chat gets an armchair once. New chats take the lowest free chair. The Lounge grows only when no chair is free, which only happens with a new chat, and compacts its chairs only on a re-pack. Lounge occupants are snapped to their new chair when a re-pack moves or reshapes the Lounge, so they move with the furniture.
  ...
  - Done goes through `finishChat(chatId)` and `finishChats(chatIds)`, both sender-guarded. `finishSteps` in `src/main/housekeeping/finish.ts` is the decision table (refuse, stop, keep or remove). Housekeeping looks up the chat and its worktree, runs `blockedReason` plus "another chat works in it", asks through the native dialog, then stops, archives and removes. `finishChats` asks once with a preview of what it will archive, remove, keep and skip.
  - The renderer marks ids as finishing before it calls main. When a finishing chat's archive patch arrives, its agent waves, walks out, and leaves a ghost holding its desk. `trips.ts` is the mover state machine (arrive, fetch, pickup, carry, load, leave: 5.2 s). Up to three desks join a trip while the truck is arriving, and later ones queue. The desk is rebuilt bare at pickup, and the ghost goes only once the truck has left, which triggers the re-pack.
  ...
  - Aaron's fixture floor went from 262 to 283 floor units, because the Lounge now has an armchair for every chat (21 instead of 15). After a relaunch where every idle chat was active in the last day, reserved desks make the floor larger still.
...
- Done (built 2026-09-24): one chat at a time, from the office. It stops the chat's processes, archives it and removes its worktree when `blockedReason` allows it; visitors get the visitor archive plus the same removal. An unsafe worktree asks "archive only and keep the worktree?" first, a busy visitor is refused, and a busy office chat offers "Stop and finish". "Clear all done" runs it for every Standby chat after one preview.
...
- [ ] A removed desk in the middle of a section becomes the free desk and gets its monitor back, while the last desk disappears. It reads as "the section shrank", but the desk the movers emptied fills again. Unit 14.
...
- [ ] Move to lounge on a stuck chat takes it out of the door queue and the monitor's list, but the drawer still shows it under Waiting for you, and it doesn't appear in Standby. Unit 14.
- [ ] Armchairs for every chat mean the Lounge shows empty chairs whenever agents are at their desks. That's the price of never re-packing on a state change. Unit 14.
