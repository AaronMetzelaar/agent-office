...
- R5. Each state reads at a glance, and "needs you" is always the loudest:
  ...
  - The desk stays reserved and reads as away: an empty chair pushed out and the monitor dimmed. It still counts toward the section's size. The reservation ends, and the desk is removed without movers, only when the chat parks after a day or is archived by any path.
  ...
  Two actions finish work by hand, for office chats and visitors alike:
  ...
  - **Done** archives the chat and cleans up its worktree under R23's rules.
  - Both sit on the agent's tag (on hover, or zoomed in) for done, idle and stuck chats, on Lounge agents, in the Standby rows and in the chat header. The Standby group also has "Clear all done", which shows what it will do in one confirmation first.
...
- R23. Cleanup: chats with no message for 3 days are suggested for cleanup. The possible actions are stop processes, archive the chat, and remove the worktree. Removing a worktree is blocked, with the reason shown, when it has uncommitted or unpushed changes. One "Clean up safe" action previews, then handles, all the safe candidates, and reports what was freed.
  ...
  - Done (R5) applies these rules to one chat. An office chat has its processes stopped and is archived, and its worktree is removed when the checks above allow it. A visitor gets the visitor archive plus the same safe removal. When removal isn't safe, Done asks first ("Uncommitted changes in <worktree>: archive only and keep the worktree?"). Working or needs-you visitors are refused. A working office chat offers "Stop and finish" behind a confirmation. "Clear all done" skips busy chats. The renderer only sends chat ids; main looks up every path itself.
