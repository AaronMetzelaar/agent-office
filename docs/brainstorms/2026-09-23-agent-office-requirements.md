...
- R3. Departments reflect where the work happens:
  ...
  - The floor re-packs only when a desk is added (a new or returning chat with no reserved desk) or removed (a Done cleanup finishes, or a reserved desk is released). A state change never re-packs it on its own.
...
- R5. Each state reads at a glance, and "needs you" is always the loudest:
  ...
  - The Lounge has one armchair per chat, never overlapping. Armchairs are assigned once and kept: a new chat takes a free one and nobody else moves, so resting and waking never resize the floor. Gaps close only when the floor re-packs, and then the occupants move with their chairs.
