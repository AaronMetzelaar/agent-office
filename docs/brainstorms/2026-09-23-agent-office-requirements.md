...
- R3. Departments reflect where the work happens:
  ...
  A section only appears while it holds desks. Working, needs-you, stuck and done-unread chats sit at theirs. A chat resting in the Lounge keeps its desk reserved until it parks or is archived (R5), so its section keeps its size. Parked chats, and chats that rest without a desk, don't count. Empty sections fold away. Inside the building the rest keep a fixed order: Marketplace, Admin, Mobile, Backend / infra, PR reviews, Research gym. Starting an agent in a hidden department brings its section back. ⌘N, the job board and the review queue can still target hidden departments.

  Sections and the building are only as big as they need to be:
  - A section holds a desk for each chat with a desk (at it, or reserved while the chat rests) plus one free desk, so there's always a desk to start from. The desks form a compact grid that grows in steps (two side by side, then 2×2, 3×2, 3×3, then wider) and shrinks as desks are removed. A growing section never moves an existing desk. A removed desk leaves a gap and nobody moves; the free desk shifts into the gap, and the grid tightens, at the next re-pack. The gym gets treadmills only for agents that run (working, or queued at the door and coming back); a done gym agent waits by the water cooler instead.
  ...
  - The floor, walls, windows and shadows hug the packed sections plus the fixed parts: his glass office with the door queue, the entrance with its cloakroom, and the Lounge. The Lounge sits beside his office or at the end of a row, wherever it packs tighter. The overview camera refits when the building changes, and frames the playground too.
  - The floor re-packs only when a desk is added (a new or returning chat with no reserved desk), a desk is removed (a Done cleanup finishes, or a reserved desk is released), or an agent arrives in a full Lounge, which adds a chair and counts as an add. A state change never re-packs it otherwise.
  - Resizes and moves animate instead of jumping. Seated agents and Lounge occupants move with their room, together with the furniture, instead of walking across the office. An agent walks only when its own spot changes: desk to Lounge, Lounge to desk, or out through the entrance.
  - Re-layout waits while he's hovering or zoomed into a section, and applies when he returns to the overview. The one exception is a newcomer who needs a desk: that desk appears at once, and the next free desk waits. A desk removed while he's zoomed in stays, bare, until then.
...
- R5. Each state reads at a glance, and "needs you" is always the loudest:
  ...
  - A done chat stays at its desk as a result waiting for him. Once he has read it (opened it and then left it by closing it, opening another chat or going back to the overview), its agent walks to the Lounge. An idle chat goes straight there.
  - The desk stays reserved and reads as away: an empty chair pushed out and the monitor dimmed. It still counts toward the section's size. The reservation ends, and the desk is removed without movers, only when the chat parks after a day, is finished with Done, or is archived in Housekeeping.
  - An agent never walks away while its chat is open. A message to a resting chat walks it back to its own reserved desk (a new desk if it had none), and it goes to Working.
  - After a relaunch, idle chats active in the last day start in the Lounge with a reserved desk. Older ones start there with no desk.
  - Visitor chats (R18) follow the same rules from their observed state: a visitor that starts working again walks back to its desk.
  - The Lounge has an armchair for each occupant, never overlapping, and hides while empty. An arriving agent takes a free armchair; if every one is taken, the Lounge grows by one, even while he's hovering or zoomed in. An agent that leaves leaves its armchair empty and nobody moves. Empty armchairs go at the next re-pack that happens anyway, and then the occupants move with their chairs.
  ...
  Actions that finish work by hand, for office chats and visitors alike:
  - **Move to lounge** is the automatic move on demand, offered for done, idle and stuck chats. The desk stays reserved, and a stuck chat leaves the door queue but stays in the inbox.
  - **Done** clears the agent's desk and stops an office chat's processes. The chat isn't archived: it moves to a Finished group in the drawer, collapsed with a count, below Standby, that shows each chat's department and "finished 2h ago". ⌘K finds finished chats, and opening one shows its transcript. A message to a finished office chat brings it back to Working at a new desk, and its agent walks in from the entrance. A finished visitor stays off the floor until it's active again under the normal visitor rules. The finished mark survives a relaunch (office.db for office chats, next to the visitor settings for visitors).
  - **Done + remove worktree** is Done plus the safe worktree removal (R23), still without archiving. It's offered only while the removal checks pass.
  - The actions sit on the agent's tag (on hover, or zoomed in) for done, idle and stuck chats, on Lounge agents, in the Standby rows and in the chat header. The Standby group also has "Clear all done", with an "also remove safe worktrees" checkbox that's off by default, and it shows what it will do in one confirmation first.
  - Archiving is only in Housekeeping (R23): the tag, the Lounge, Standby and the chat header don't offer it.
  - Done plays out in about five seconds. The agent waves, walks out through the entrance and disappears. A moving truck backs up to the street by the entrance; it only exists while it's needed. Three movers in overalls and caps hop out, carry the monitor, the plant and a box from the desk to the truck and hop back in, and the truck speeds off with a puff. Then the desk is removed and the floor re-packs under the rules above. Dones that arrive together share one truck (up to three desks), and the rest wait for the next trip. Neither starting an agent nor Done moves the camera.
...
- R23. Cleanup: chats with no message for 3 days are suggested for cleanup. The possible actions are stop processes, archive the chat, and remove the worktree. Removing a worktree is blocked, with the reason shown, when it has uncommitted or unpushed changes. One "Clean up safe" action previews, then handles, all the safe candidates, and reports what was freed.
  - Visitors (R18) follow the same rules. They count in the worktree total and disk size, "Clean up safe" includes them, and the summary says how many were visitors. The office never stops a visitor's processes; a `claude` process working in the worktree blocks removal instead.
  - A visitor's RAM shows only when a `claude` process carries its session id in its arguments (the desktop app's `--resume=<id>`). Otherwise the row says "RAM: in desktop app" (or "in terminal").
  - Done (R5) finishes a chat without archiving it: an office chat has its processes stopped, a visitor is only marked finished. "Done + remove worktree" also removes the worktree when the checks above allow it, and if they no longer do it asks first ("Uncommitted changes in <worktree>: finish only and keep the worktree?"). Working or needs-you visitors are refused. A working office chat offers "Stop and finish" behind a confirmation. "Clear all done" skips busy chats. The renderer only sends chat ids; main looks up every path itself.
  - Finished chats stay eligible for the cleanup suggestions above; archiving happens only here.
